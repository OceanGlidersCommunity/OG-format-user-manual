...
PHASE describes the glider behaviors when at sea. The different behaviors are described in the phase vocabulary (ascent, descent, surfacing, parking, inflection, etc.) PHASE is derived from the glider's engineering state, so describes what the glider is attempting to do. So, when the glider is on the surface and starts to adjust its pitch and ballast to begin its next descent, the PHASE will be 1 (descent) even though the glider is stationary on the surface. In this way, PHASE can be considerd a normalisation table for engineering behavious of the various glider platforms.
