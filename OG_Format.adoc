...
This is the current version of the format. Previous versions:


*  https://oceangliderscommunity.github.io/OG-format-user-manual/OG_Format-v1.0.0.html[v1.0.0]. The initial working release of the OceanGliders format, as presented at the IUGC2024 conference.
...
* OG1.0 follows the IODE quality control, IODE_QCfootnote:[https://vocab.nerc.ac.uk/collection/L27/current/IODE_QC/], flagging schemafootnote:[http://dx.doi.org/10.25607/OBP-6] 
* Geospatial variables and along-track positioning variables are mandatory.
* Variables use the standard_name attribute following CF 1.10 taken from the CF standard names tablefootnote:[https://cfconventions.org/Data/cf-standard-names/current/src/cf-standard-name-table.xml].
...
|mandatory |**ex.:** http://vocab.nerc.ac.uk/collection/L06/current/27/[_http://vocab.nerc.ac.uk/collection/L06/current/27/_]
|id a|
...
|contributing_institutions_role_vocabulary |The controlled vocabulary of the role used in the institution's role. Multiple vocabularies are separated by commas. |Operator vocabulary is mandatory |**ex.:** http://vocab.nerc.ac.uk/collection/W08/current/[_http://vocab.nerc.ac.uk/collection/W08/current/_]
...
|N_MEASUREMENTS |N_MEASUREMENTS = unlimited; | The sole dimension of the file. the length of N_MEASUREMENTS is equal to the total number of unique timestamps contained in the dataset.
|=================================================================================================================================================================================================================================================================
...
PLATFORM_MODEL: platform_model_vocabulary = “http://vocab.nerc.ac.uk/
...
PLATFORM_MAKER: platform_maker_vocabulary = “http://vocab.nerc.ac.uk/
...
PHASE describes a glider’s behavior using a defined vocabulary of states—such as ascent, descent, surfacing, parking, and inflection—and serves as a normalization layer for interpreting engineering behaviors across different glider platforms. It is derived from the glider’s engineering state and reflects what the glider is attempting to do. Sometimes, the observed behavior may lag behind the engineering state—for example, when the glider adjusts its ballast and pitch in preparation for descent, the PHASE will be set to 1 (descent), even though the glider may not begin sinking immediately.```

Normalisation tables to translate from the nav states of several glider models into the OG PHASE variable can be found at https://github.com/OceanGlidersCommunity/OG-format-user-manual/blob/main/vocabularyCollection/phase_translation_tables.md
...
PROFILE_NUMBER:profile_number_calculation_method = "PROFILE_NUMBER increments by one each time the glider starts an ascending or descending profile. PROFILE_NUMBER is one at the start of the first ascending or descending profile in the datasets. When the glider is not in one of these states, PROFILE_NUMBER is NaN”;
...
PROFILE_DIRECTION:profile_direction_calculation_method = "PROFILE_DIRECTION is 1 when a glider is descending and -1 when the glider is ascending. Otherwise it is 0”;

PROFILE_DIRECTION:_FillValue = 0b ;
...
Where <sensor_type> is a controlled vocabulary. See the https://oceangliderscommunity.github.io/OG-format-user-manual/vocabularyCollection/tableOfControlledVocab.html[vocabulary guidance] for more information. A list of sensor types can be found at http://vocab.nerc.ac.uk/collection/L05/current/[NERC L05 collection] and more can be requested via the https://github.com/nvs-vocabs/L05/issues[corresponding GitHub repository].

To retain compliance with https://cfconventions.org/Data/cf-conventions/cf-conventions-1.10/cf-conventions.html#_naming_conventions[CF variable naming conventions] and the OG1 convention of all variables being upper case, the name of the sensor variable should be all upper-case and replace any characters other than letters and numbers with underscores. e.g. a dissolved gas sensor (https://vocab.nerc.ac.uk/collection/L05/current/351/[L05/351]) with sensor serial number 2025-0123 would have the variable name SENSOR_DISSOLVED_GAS_SENSORS_2025_0123. 
...
sensor_model_vocabulary: “http://vocab.nerc.ac.uk/collection/L22/current/TOOL1745”
...
<PARAM>:long_name = The long_name is the Definition field of the variable as defined in the OG1 vocabulary.; - mandatory
